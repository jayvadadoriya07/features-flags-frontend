...
function Dashboard() {
     const data = [
    {
      ...
      role: ["viewer", "admin"], // All roles can see this
    },
    {
      title: "Kanban",
      icon: (
  ...
  useEffect(() => {
    flagsmith.identify("jay", { role: "viewer" });

    setUserRole("viewer");
  }, []);

  const filteredMenuItems = data.filter((item) => {
    if (flags.menu_access && flags.menu_access.enabled) {
      return item.role.includes(userRole);
    }
    return true;
  });

  const hasPermission = (featureFlag: string) => {
